...
local function fill_rec(total, containers, start_at, result)
	...
	for i = start_at, #containers do
		...
		fill_rec(total - container, containers, i+1, result)
		table.remove(result)
	end
...
local function fill(total, containers)
	return coroutine.wrap(function() fill_rec(total, containers, 1, {}) end)
end
